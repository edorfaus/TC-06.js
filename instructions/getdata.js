...
class GetdataInstruction
{
	constructor(outRegister) {
	...
	run(processor, instruction) {
		...
		let result = processor.deviceBus.getData(port, value);
		if (typeof result === 'undefined') {
			return Processor.RUN_ERROR;
		}
		let status = processor.registers.write(this.outRegister, result);
